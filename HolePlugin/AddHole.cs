...
namespace HolePlugin
{
    ...
    public class AddHole : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            if (ovDoc == null)
            {
                TaskDialog.Show("Error", "Не найден файл ОВ");
                return Result.Cancelled;
            }

            FamilySymbol familySymbol = new FilteredElementCollector(arDoc) //проверка наличия семейства "Отверстие"
            ...
            {
                TaskDialog.Show("Error", "Не найдено семейство");
                return Result.Cancelled;
            }
            ...
            using (Transaction ts = new Transaction(arDoc, "Create holes"))
            {
                ts.Start();
                ...
                {
                    Curve curve = (d.Location as LocationCurve).Curve;
